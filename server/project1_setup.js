...
const loadData = async () => {
    ...
    try {
        ...
        let results = await dbRtns.deleteAll(db, cfg.collection);
        logs = `Deleted ${results.deletedCount} existing documents from the ${cfg.collection} collection. `;
        ...
        results = await dbRtns.addMany(db, cfg.collection, alerts);
        logs += `Added ${results.insertedCount} documents to the ${cfg.collection} collection.`
    }
    catch (err) {
        logs = err;
        console.log(err);
